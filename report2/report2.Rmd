...
O experimento consiste em estimar a constante de elasticidade de uma mola. Para isto, uma mola foi prendida a um suporte que a manteve na posição vertical sendo suspensa pela sua extremidade superior. Com a mola nesta posição, as seguintes etapas foram realizadas:
...
A próxima tabela apresenta os dados de *k* para a expansão obtida com o objeto de massa 250g em relação a mola sem objeto, valores aproximados também seriam obtidos se fizer calcular *k* para os demais objetos.

```{r echo=F}
dfK250 <- (dfExpansaoForca[,c(1,2,3)] / 1000)[6,]
dfK250$Media <- mean(dfK250$Aluno1, dfK250$Aluno2, dfK250$Aluno3)
rownames(dfK250) <- c("$k_{250g}$")
knitr::kable(dfK250, digits = 5, col.names=c("Aluno1", "Aluno2", "Aluno3", "Média"), caption = 'Constante elástica para o objeto de 250g')
```
